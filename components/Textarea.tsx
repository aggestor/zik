...
const Textarea: FunctionComponent<Props> = ({label, placeholder,value, name,error,onChange}) =>{
    return <div>
        <div className={`focus-within:text-blue-600 transition-all  w-full duration-[400] focus-within:font-semibold my-2 ${error && error[name] ? ' text-red-600' : 'text-gray-600'} flex flex-col`}>
            ...
            <textarea value={value} onChange={onChange} className={`transition-all duration-300 focus:font-normal resize-none h-[100px] w-full focus-within:ring-2 focus:ring-blue-600 px-1.5 ${error && error[name] && 'border-red-600'} rounded-lg outline-none text-gray-700`} name={name} id={name} placeholder={placeholder}></textarea>
    </div>
